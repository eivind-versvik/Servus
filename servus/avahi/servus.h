...
private:
    ...
    static void _resolveCBS(AvahiServiceResolver* resolver, AvahiIfIndex,
                            ...
                            const char* host, const AvahiAddress* address, const uint16_t port,
                            AvahiStringList* txt, AvahiLookupResultFlags flags,
                            void* servus)
    {
        ((Servus*)servus)->_resolveCB(resolver, event, name, host, address, port, txt, flags);
    }
                    ...
                    const AvahiLookupResultFlags flags)
    {
        ...
        {
            char addr_char[AVAHI_ADDRESS_STR_MAX];
            avahi_address_snprint(addr_char, sizeof(addr_char), address);
            ValueMap& values = _instanceMap[name];
            values["servus_host"] = host;
            values["servus_address"] = addr_char;
